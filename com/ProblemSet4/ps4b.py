...
def compChooseWord(hand, wordList, n):
    ...
    wordsInHand = ''
    for i in hand.keys():
        for j in range(hand.get(i)):
            wordsInHand = wordsInHand+i
    wordLength = len(wordsInHand)
    maxScorePossible = getWordScore(wordsInHand, len(wordsInHand))
    maxScore=0
    bestWord='None'
    for word in wordList:
        currentWordScore = getWordScore(word,n)
        if isValidWord(word, hand, wordList) and (currentWordScore > maxScore):
            maxScore = currentWordScore
            bestWord = word
            if(maxScore == maxScorePossible):
                break
    return bestWord
#
...
def compPlayHand(hand, wordList, n):
    ...
    while calculateHandlen(hand) > 1:
        ...
        if isValidWord(compWord,hand,wordList):
            wordScore = getWordScore(compWord,n)
            totalScore = totalScore+wordScore
            print "\""+compWord+"\""+" earned "+str(wordScore)+" points. Total: "+str(totalScore)+" points\n"
...
def playGame(wordList):
    ...
    while True:
        ...
        if userInput == 'e':
            ...
            if not bool(lastPlayedWordList):
                print "You have not played a hand yet. Please play a new hand first!\n"
            else:
                chooseUserAndStart(lastPlayedWordList,wordList)
        elif userInput == 'n':
            lastPlayedWordList = dealHand(HAND_SIZE)
            chooseUserAndStart(lastPlayedWordList,wordList)
        else:
            print "Invalid command.\n"

def chooseUserAndStart(lastPlayedWordList,wordList):
    while True:
        userInput = raw_input("Enter u to have yourself play, c to have the computer play: ")
        if userInput == 'u':
            playHand(lastPlayedWordList,wordList,HAND_SIZE)
            break
        elif userInput == 'c':
            compPlayHand(lastPlayedWordList, wordList, HAND_SIZE)
            break
        else:
            print "Invalid command.\n"
#
