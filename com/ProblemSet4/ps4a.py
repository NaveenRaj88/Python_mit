...
def getWordScore(word, n):
    ...
    word = word.lower()
    for i in word:
        wordScore = wordScore+SCRABBLE_LETTER_VALUES.get(i)
    bonus = 0
    if(len(word) == n):
        bonus = 50
    return wordScore * len(word)+bonus
...
def updateHand(hand, word):
    ...
    for i in word:
        updatedHand[i]= updatedHand.get(i)-1
    return updatedHand
...
def calculateHandlen(hand):
    ...
    totalLetters=0
    for i in hand.keys():
        totalLetters = totalLetters+hand.get(i,0)
    return totalLetters
